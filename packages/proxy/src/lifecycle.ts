...
export interface ProxyState {
  ...
  cliVersion?: string;
  /**
   * Admin token for the write endpoints (POST /api/active). Generated by the
   * daemon at startup, persisted here so the GUI can read a stable token
   * across daemon restarts. Optional: foreground/non-daemon startProxy callers
   * omit it, in which case admin write endpoints return 501 admin_disabled.
   */
  adminToken?: string;
}
