...
import { convertOpenAISSEToAnthropic, resetSSEState } from "../../src/protocol/openai-to-anthropic-sse.js";

describe("convertOpenAISSEToAnthropic", () => {
  beforeEach(() => {
    resetSSEState();
  });
  ...
  it("should transition idle -> started -> text_block_started -> idle", () => {
    ...
    const result1 = convertOpenAISSEToAnthropic(chunk1, "claude-sonnet-4-6");
    ...
    const result2 = convertOpenAISSEToAnthropic(chunk2, "claude-sonnet-4-6");
  ...
  it("should emit message_start + content_block_start on first chunk with role", () => {
    ...
    const result = convertOpenAISSEToAnthropic(chunk, "claude-sonnet-4-6");
  ...
  it("should emit content_block_delta for content", () => {
    ...
    convertOpenAISSEToAnthropic(startChunk, "claude-sonnet-4-6");

    // Content delta
    const contentChunk = 'data: {"id":"chatcmpl-3","choices":[{"delta":{"content":"Hello world"}}]}\n\n';
    const result = convertOpenAISSEToAnthropic(contentChunk, "claude-sonnet-4-6");
  ...
  it("should emit content_block_stop + message_delta + message_stop on finish_reason", () => {
    ...
    const result = convertOpenAISSEToAnthropic(finishChunk, "claude-sonnet-4-6");
    ...
    const lines = result!.split("\n");
    const msgDeltaLine = lines.find((l) => l.includes("message_delta") && l.startsWith("data:"));
    // The data line follows the event line, find the one with stop_reason
    const dataLines = lines.filter((l) => l.startsWith("data: "));
  ...
  it("should handle [DONE] and emit proper closing events", () => {
    ...
    const result = convertOpenAISSEToAnthropic(doneChunk, "claude-sonnet-4-6");
  ...
  it("resetSSEState should clear state between streams", () => {
    // First stream
    ...
    convertOpenAISSEToAnthropic(startChunk, "claude-sonnet-4-6");
    convertOpenAISSEToAnthropic(contentChunk, "claude-sonnet-4-6");
    convertOpenAISSEToAnthropic(finishChunk, "claude-sonnet-4-6");
    ...
    const result = convertOpenAISSEToAnthropic(startChunk2, "claude-sonnet-4-6");
  ...
  it("should skip invalid JSON lines", () => {
    ...
    const result = convertOpenAISSEToAnthropic(badChunk, "claude-sonnet-4-6");
    expect(result).toBeNull();
  });
