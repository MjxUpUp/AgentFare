...
describe("E2E: Streaming cost tracking with token extraction", () => {
  ...
  it("should extract tokens from OpenAI SSE stream with usage field and record them", async () => {
    ...
    uninstall = installFetchPatch({ handler: mockHandler, costTracker, registry });
  ...
  it("should correctly calculate cost from extracted tokens", async () => {
    ...
    // registry is provided to installFetchPatch, so originalModelEntry resolves to
    // gpt-5.5; originalCost uses gpt-5.5 pricing on the extracted tokens and
    // savings = originalCost - actualCost (downgrade powerful → fast saves money).
    const expectedOriginalCost =
      (10 / 1_000_000) * originalModel.pricing.inputPerMillion +
      (20 / 1_000_000) * originalModel.pricing.outputPerMillion;
    expect(entry.actual_cost).toBeGreaterThan(0);
    expect(entry.original_cost).toBeCloseTo(expectedOriginalCost, 10);
    expect(entry.savings).toBeCloseTo(expectedOriginalCost - expectedActualCost, 10);
  });
});
